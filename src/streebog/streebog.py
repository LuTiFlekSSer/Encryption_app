...
class Streebog:
    ...
    @staticmethod
    def _P(x: list[int]) -> list[int]:
        return [x[tau[i]] for i in range(len(x))]

    @staticmethod
    def _S(x: list[int]) -> list[int]:
        return [pi[elem] for elem in x]

    @staticmethod
    def _X(x: list[int], k: list[int]) -> list[int]:
        return [x[i] ^ k[i] for i in range(len(x))]

    @staticmethod
    def _g(hsh: list[int], mess: list[int], sum_len: list[int]) -> list[int]:
        ks: list[list[int]] = [Streebog._L(Streebog._P(Streebog._S(Streebog._X(hsh, sum_len))))]
        for i in range(12):
            ks.append(Streebog._L(Streebog._P(Streebog._S(Streebog._X(ks[-1], C[i])))))

        hsh = Streebog._X(hsh, mess)
        for i in range(12):
            mess = Streebog._L(Streebog._P(Streebog._S(Streebog._X(mess, ks[i]))))

        return Streebog._X(Streebog._X(mess, ks[12]), hsh)

    @staticmethod
    def _add_list(x: list[int], y: list[int]) -> list[int]:
        overlap = 0
    ...
    def calc_hash(data: bytes) -> bytes:
        ...
        while len(data) >= 64:
            mess = data[-64:]
            data = data[:-64]
            hsh = Streebog._g(hsh, mess, sum_len)
            sum_len = Streebog._add_list(sum_len, list((512).to_bytes(byteorder='big', length=64)))
            sum_mess = Streebog._add_list(sum_mess, mess)

        mess = [0] * (64 - len(data) - 1) + [1] + data
        hsh = Streebog._g(hsh, mess, sum_len)
        sum_len = Streebog._add_list(sum_len, list((len(data) * 8).to_bytes(byteorder='big', length=64)))
        sum_mess = Streebog._add_list(sum_mess, mess)
        hsh = Streebog._g(hsh, sum_len, [0] * 64)
        hsh = Streebog._g(hsh, sum_mess, [0] * 64)
