...
DWORD WINAPI encrypt_kyznechik_ecb_thread(LPVOID raw_data) {
    ...
    for (uint64_t i = 0; i < total; ++i) {
        ...
        for (uint64_t j = 0; j < BUF_SIZE; j += 16) {
            kyznechik_encrypt_data((const uint8_t**)data->Ks, input_file_info.data + j, output_file_info.data + j);
        }
...
uint8_t encrypt_kyznechik_ecb(
    ...
    *total_steps = file_size.result / 16;
    ...
    uint8_t result = check_files(input_file, output_file, disk_space.result,
    ...
    for (uint16_t i = 0; i < num_threads - 1; ++i) {
        threads_data[i] = (thread_data){
            ...
            .lock = &lock, .input_file = input_file, .output_file = output_file, .error = &error, .Ks = Ks
        };
    }
    ...
    result = encrypt_last_bytes((const uint8_t**)Ks, &(file_block_info){
                                    .file = output_file, .offset = *total_steps * 16, .data = delta, .data_size = 16
                                });
